...
### rvi-vendor-request-wizard-debugger
- Path: ~/.config/opencode/skills/rvi-vendor-request-wizard-debugger/SKILL.md
- Trigger: >
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.
...
### sync-dev-tiger
- Path: ~/.config/opencode/skills/sync-dev-tiger/SKILL.md
- Trigger: >
- Rules:
  - **Always fetch before any operation** — ensures we have latest remote state
  - **Merge commit ONLY** — never squash or rebase for this workflow
  - **Auto-merge requires branch protection on main** — verify with user if first run
  - **Conflict resolution is manual** — stop and inform user, never auto-resolve
  - **Push before creating PR** — PR needs the branch on remote
